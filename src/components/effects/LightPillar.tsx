...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    const isLowEndDevice =
      isMobile || (navigator.hardwareConcurrency != null && navigator.hardwareConcurrency <= 4);

    let effectiveQuality = quality as 'low' | 'medium' | 'high';
    ...
    // Transparent background so areas with no light don't cover content behind the canvas
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        // Use luminance as alpha: dark areas (no light) become transparent,
        // bright light beams become opaque — allows the ambient behind to show through.
        float luma = dot(result, vec3(0.2126, 0.7152, 0.0722));
        float alpha = clamp(luma * 8.0, 0.0, 1.0);
  ...
  useEffect(() => {
    ...
    materialRef.current.uniforms.uTopColor.value.set(c.r, c.g, c.b);
  }, [topColor]);

  useEffect(() => {
    if (!materialRef.current) return;
    const c = new THREE.Color(bottomColor);
    materialRef.current.uniforms.uBottomColor.value.set(c.r, c.g, c.b);
  }, [bottomColor]);
  ...
  useEffect(() => { if (materialRef.current) materialRef.current.uniforms.uLightMode.value = lightMode ? 1 : 0; }, [lightMode]);
  useEffect(() => {
    if (!materialRef.current) return;
